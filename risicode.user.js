...
// @icon         https://image.jeuxvideo.com/stickers/p/1klb
// @grant        GM_setValue
// @grant        GM_getValue
...
new class RisiCode {
    ...
    get textarea () {
        if(this._textarea !== undefined) {
            ...
            this._textarea = document.getElementById("message_topic");
            return this._textarea;
        }
    }
    ...
    replace (str1, str2) {
        this.textarea.value = this.textarea.value.replace(str1, str2);
    }
    ...
    insert (str) {
        ...
        this.textarea.value = this.textarea.value.substring(0, startPos) + " " + str + " " + this.textarea.value.substring(endPos, this.textarea.value.length);
    }
    ...
    pushStickersWebedia () {
        ...
        this.stickers = [
            ...
            [":photo:", "[[sticker:p/1ljn]]"], [":bescherelle:", "[[sticker:p/1ljo]]"], [":+1:", "[[sticker:p/1ljp]]"], [":master:", "[[sticker:p/1ljr]]"], [":thug:", "[[sticker:p/1ljq]]"], [":go mp:", "[[sticker:p/1rzs]]"], [":tant pis:", "[[sticker:p/1rzw]]"], [":fort:", "[[sticker:p/1lgd]]"],
            ...
            [":bbq:", "[[sticker:p/1f8f]]"], [":sup:", "[[sticker:p/1f88]]"], [":burp:", "[[sticker:p/zu7]]"], [":sombrero:", "[[sticker:p/1lmi]]"], [":sleep:", "[[sticker:p/1lml]]"], [":cute2:", "[[sticker:p/1lmh]]"], [":pote:", "[[sticker:p/1lmj]]"], [":triste:", "[[sticker:p/1lmk]]"], [":hotte:", "[[sticker:p/1lmm]]"], [":hey:", "[[sticker:p/1lmn]]"], [":titeuf:", "[[sticker:p/1lmo]]"], [":wc:", "[[sticker:p/1lmp]]"],
            ...
            [":like a boss:", "[[sticker:p/1lt9]]"], [":salut2:", "[[sticker:p/1jc3-fr]]"], [":gg:", "[[sticker:p/1li5]]"], [":star:", "[[sticker:p/1li4]]"],
            [":allez:", "[[sticker:p/1n1m]]"], [":but:", "[[sticker:p/1n1n]]"], [":deprime2:", "[[sticker:p/1n1o]]"], [":carton rouge:", "[[sticker:p/1n1s]]"], [":victoire:", "[[sticker:p/1n1t]]"], [":what:", "[[sticker:p/1n1q]]"]
        ];
    }
        ...
        let next = () => {
            ...
            data.forEach((sticker, index) => {
                liste.push({
                    ...
                    image_risibank: "https://risibank.fr/cache/stickers/d" + (Math.floor(sticker.id/100)) + "/" + sticker.id + "-thumb." + sticker.ext,
                    tags: sticker.tags
                });
            });
    ...
    htmlStickersWebedia () {
        let html = `<div style="margin-bottom: 5px; border-bottom: 1px solid; padding-bottom: 5px; font-size: 1.2em;">Stickers Webedia <a style="float: right; cursor: pointer" data-str-1="[Afficher]" data-str-2="[Masquer]" id="toggle-stkswebedia">[Afficher]</a></div>`;
        ...
        for(let sticker of this.stickers)
        {
            ...
            {
                afficher = '<img src="http://image.jeuxvideo.com/stickers/'+RegExp.$1+'" height="44px;">';
            }
            else
            {
                afficher = sticker[1];
    ...
    htmlAfficherStickerPerso (perso) {
        ...
        if(/^http(s)?:\/\/image.noelshack.com\/(fichiers|minis)\/([0-9]+)\/([0-9]+)\/([0-9]+)-([0-9a-zA-Z-]+).(png|jpg|jpeg|gif)$/.test(perso[1]) ||
           /^http(s)?:\/\/image.noelshack.com\/(fichiers|minis)\/([0-9]+)\/([0-9]+)\/([0-9]+)\/([0-9]+)-([0-9a-zA-Z-]+).(png|jpg|jpeg|gif)$/.test(perso[1])) {
            afficher = '<img data-real-src="'+perso[1]+'" width="68" height="51" class="risicode-bt-st" data-stk="'+perso[1]+'" style="cursor:pointer">';
        }
            ...
            afficher = '<img data-real-src="'+link+'" width="68" height="51" class="risicode-bt-st" data-stk="'+link+'" style="cursor:pointer">';
        }
        else if(/^\[\[sticker:([0-9a-zA-Z\/]+)\]\]$/.test(perso[1])) {
            afficher = '<img src="image.jeuxvideo.com/stickers/'+RegExp.$1+'" height="44px;">';
        }
        else {
    ...
    callbackStickersPerso () {
        ...
        this.afficheStickersSync()
    }

    afficheStickersSync () {
        let img = document.querySelector("*[data-real-src]")
        if(img) {
            let src = img.getAttribute("data-real-src")
            if(/^https?:\/\/image\.noelshack\.com\/fichiers\/(.+)\./.test(src)) {
                src = `http://image.noelshack.com\/minis\/${RegExp.$1}.png`
            }
            img.onload = () => this.afficheStickersSync()
    ...
    htmlDivStickerRisibank (sticker) {
        return `<div id="risibank-sticker-${sticker.id}" style="margin: 5px;  display: inline-block; min-width: 6%; padding:  5px;border: 1px dashed;border-radius:  3px; cursor: pointer" class="risicode-bt-risibank" data-stk="${sticker.image_noelshack}">
                          <div style="width: 100%; display: block; text-align: center; margin: 0"><img onerror="this.onerror = function () {this.parentNode.parentNode.remove()};this.src='${sticker.image_noelshack}';" src="${sticker.image_risibank}" width="64px" title="${sticker.tags}"></div>
